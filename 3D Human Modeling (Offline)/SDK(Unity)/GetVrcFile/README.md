# Method - OfflineClient.GetVrcFile  
Get the specified RelationId VrcFile and return the file name.  

```cs
public static Response GetVrcFile(string vrcId, string password, string relationId = "");
```
## Parameters
- vrcId `string`  
...
If relationId is empty, then return the latest VrcFile.

## Returns
`Response`  
 The response message.Gets the `VrcFileName` of a response message.
...
| VrcFileName    | string     |The 3D Model File Name |
